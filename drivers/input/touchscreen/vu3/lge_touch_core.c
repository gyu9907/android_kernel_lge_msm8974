...
#ifdef CONFIG_LGE_SECURITY_KNOCK_ON
	if((int)simple_strtoul(&ts->fw_info.ic_fw_version[1], NULL, 10)<26){
		if(buf !=0x04){
			mutex_unlock(&i2c_suspend_lock);
			mutex_unlock(&ts->irq_work_mutex);
...
//	if( type == 1 ){
//#endif
	if (value == touch_gesture_enable || (ts_suspend == 0) )
		goto no_action;
...
//    static int suspend = 0;
	int value;
	sscanf(buf, "%d", &value);
	TOUCH_INFO_MSG("store_touch_gesture temp %d\n\n", value);
	/*
	int type = 0;
	int value[4] = {0};
...
//			set_wakeup_mode(ts, value[0]);
			set_wakeup_mode(ts, value);
			
//            touch_device_func->lpwg(ts->client, LPWG_ENABLE, value[0], NULL);
			mutex_lock(&ts->irq_work_mutex);
            touch_device_func->lpwg(ts->client, LPWG_ENABLE, value, NULL);
