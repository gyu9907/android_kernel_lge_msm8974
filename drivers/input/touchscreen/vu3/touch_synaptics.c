...
int synaptics_ts_ic_ctrl(struct i2c_client *client, u8 code, u16 value)
{
	...
	case IC_CTRL_DOUBLE_TAP_WAKEUP_MODE:
			...
			case 1: /* touch double-tap enable */
			{
					...
					*(r_mem+0) = 0x1;
#if defined(CONFIG_LGE_Z_TOUCHSCREEN)||defined(A1_only) || defined(CONFIG_LGE_VU3_TOUCHSCREEN)
					*(r_mem+1) = 0x28;
#else
					*(r_mem+1) = 0x14;
#endif
...
#if defined(A1_only) || defined(CONFIG_LGE_VU3_TOUCHSCREEN)
					if (touch_i2c_write(ts->client, WAKEUP_GESTURE_ENABEL_REG, (6), r_mem) < 0) {
						TOUCH_ERR_MSG("WAKEUP_GESTURE_ENABEL_REG write fail");
						if (r_mem != NULL)
							kfree(r_mem);
...
#endif
#if defined(CONFIG_LGE_VU3_TOUCHSCREEN)
					*(r_mem+3) = 0xA;//max active duration 5 sec
					*(r_mem+4) = 0x2;//timer 200ms
					*(r_mem+5) = 0x2;//max active duration time out 200 ms
#else
					*(r_mem+3) = 0x6;//max active duration 3 sec
					*(r_mem+4) = 0x2;
					*(r_mem+5) = 0x2;
#endif
#if defined(CONFIG_LGE_Z_TOUCHSCREEN)
